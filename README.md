...
- **暴力拆解防御**: 高危认证路由强绑定 `Depends(RateLimiter)`，拦截算力耗尽攻击。
...
| 表名 | 所属端 | 用途 | 关键字段 |
|------|--------|------|---------|
| `users` | C端 | 买家核心账号 | phone_number, email, username, hashed_password, is_active |
| `user_profiles` | C端 | 用户档案 (1:1) | real_name_enc (密文), identity_card_enc (密文) |
| `user_socials` | C端 | 三方绑定 (N:1) | platform, openid, unionid, extra_data (JSONB) |
...
- [ ] **多端跨域桥接：** 建立通用的苹果 ID / 微信免密等多平台聚合体系。
- [ ] **活体与二次验证风控：** 对极其敏感行为进行 OTP 短信、设备信誉机制等二次验签挂载闭环。
- [ ] **基于业务维度的 C端特权分级 (`user_levels`)：** 用动态会员特权制度替换传统的代码堆叠，形成梯队式权益模型。
- [ ] **单元自动化覆盖：** 采用 PyTest 与模拟高并发压测，在 `tests/` 目录下完成防暴撞击与 Token 窃取的全套用例安全防线。
