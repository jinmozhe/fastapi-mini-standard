...
| 缓存 | Redis (Async) | Refresh Token 存储 + Token Rotation |
...
| 刷新令牌 | `POST` | `/api/v1/auth/refresh` | Token Rotation：旧 Token 销毁，签发新对 |
| 用户登出 | `POST` | `/api/v1/auth/logout` | 销毁 Redis 中的 Refresh Token |

- **Refresh Token 策略**: 高熵随机串存入 Redis，7 天过期，一次性使用 (Rotation 防重放)
- **安全掩码**: 登录失败统一返回 `auth.invalid_credentials`，防止用户名枚举攻击
...
| 特性 | 实现细节 |
|------|---------|
| 密码存储 | Argon2id 哈希 (`pwdlib`)，异步封装避免阻塞事件循环 |
| 身份认证 | JWT Access Token (HS256, 30 分钟有效) |
| 令牌续期 | Refresh Token 存入 Redis (7 天有效)，Token Rotation 一次性策略 |
